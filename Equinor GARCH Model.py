...
tickerDf = tickerData.history(interval='1d', start='2008-1-1', end='2023-12-30')
close_prices = tickerDf['Close']
In_Sample = 100 * close_prices.pct_change().dropna()
...
Out_Sample = 100 * close_prices_out.pct_change().dropna()
...
train_size = len(In_Sample) - test_size

for i in range(test_size):
    train = In_Sample[:train_size+i]
...
rolling_predictions = pd.Series(rolling_predictions, index=In_Sample.index[-test_size:])

# Calculate actual volatility (using absolute returns as a proxy)
actual_volatility = In_Sample[-test_size:].abs()
...
forecast_dates = pd.date_range(start=In_Sample.index[-1] + pd.Timedelta(days=1), periods=forecast_horizon)
